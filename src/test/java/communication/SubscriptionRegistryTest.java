...
public class SubscriptionRegistryTest {
	...
	public void testSubscribeTwoSameAddressUnsubscribe() {
		...
		subscriptionRegistry.subscribeTo("tcp://localhost", 8081, "secret", EncryptionAlgorithm.AES,
				null);
		subscriptionRegistry.subscribeTo("tcp://localhost", 8082, "secret", EncryptionAlgorithm.AES,
				null);
	...
	public void testSubscribeSamePortOneAddress() {
		...
		subscriptionRegistry.subscribeTo("tcp://localhost", 8081, "secret", EncryptionAlgorithm.AES,
				null);
		assertEquals(1, subscriptionRegistry.getNumberOfActiveSubscriptions());
		assertNull(subscriptionRegistry.subscribeTo("tcp://localhost", 8081, "secret",
				EncryptionAlgorithm.AES, null));
		assertEquals(1, subscriptionRegistry.getNumberOfActiveSubscriptions());
	...
	public void testSubscribeSamePortDifferentAddress() {
		...
		subscriptionRegistry.subscribeTo("tcp://10.0.1.1", 8081, "secret", EncryptionAlgorithm.AES,
				null);
		assertEquals(1, subscriptionRegistry.getNumberOfActiveSubscriptions());
		subscriptionRegistry.subscribeTo("tcp://10.0.1.2", 8081, "secret", EncryptionAlgorithm.AES,
				null);
		assertEquals(2, subscriptionRegistry.getNumberOfActiveSubscriptions());
